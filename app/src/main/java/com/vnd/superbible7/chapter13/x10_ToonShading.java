...
public class x10_ToonShading extends GLSurfaceView implements GLSurfaceView.Renderer, GestureDetector.OnDoubleTapListener, GestureDetector.OnGestureListener {
    ...
    public x10_ToonShading(Context context) {
        super(context);
    ...
    private void initialize(GL10 gl) {
        ...
        final String fragmentShaderSourceCode =
                "#version 320 es" +
                        ...
                        "vec3 light_pos = vec3(30.0, 30.0, 100.0);" +
                        "in vec3 oNormal;" +
                        "in vec3 oView;" +
                        "out vec4 fragColor;" +
                        "void main(void)" +
                        ...
                        "vec3 L = normalize(light_pos - oView);" +
                        // Simple N dot L diffuse lighting calculation
                        "float tc = pow(max(0.0, dot(N, L)), 5.0);" +
                        // Sample from cell shading texture
                        "fragColor = texture(textureSampler, vec2(tc, 0.5)) * (tc * 0.8 + 0.2);" +
                        "}";
        ...
        texture[0] = textureHelper.generateToonTexture();
