...
public class x3_TextureViewer extends GLSurfaceView implements GLSurfaceView.Renderer, GestureDetector.OnDoubleTapListener, GestureDetector.OnGestureListener {
    private final TextureHelper textureHelper;
    private final float[] perspectiveProjectionMatrix = new float[16];
    private final int[] texture = new int[512];
    ...
    public x3_TextureViewer(Context context) {
        super(context);
    ...
    public boolean onSingleTapConfirmed(@NonNull MotionEvent event) {
        ...
        if (x < screenWidth / 2) {
            textureIndex = ((textureIndex - 1) % numberOfImages + numberOfImages) % numberOfImages;
        } else {
            textureIndex = (textureIndex + 1) % numberOfImages;
        }
        return true;
    }
    ...
    private void initialize(GL10 gl) {
        ...
        final String fragmentShaderSourceCode =
                "#version 320 es" +
                        ...
                        "void main(void)" +
                        "{" +
                        // Flip the y-coordinate and normalize by screen size
        ...
        // Get the resource IDs and load texture
        for (Field field : R.raw.class.getFields()) {
            try {
                if (!field.getName().equals("smiley")) {
                    texture[numberOfImages] = textureHelper.load2DTexture(field.getInt(null));
                    numberOfImages++;
                }
            } catch (IllegalAccessException e) {
                System.out.println("VND: illegal access exception: " + e);
                uninitialize();
            }
        }
    ...
    private void display() {
        ...
        GLES32.glBindTexture(GLES32.GL_TEXTURE_2D, texture[textureIndex]);
