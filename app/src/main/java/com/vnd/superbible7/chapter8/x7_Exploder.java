...
public class x7_Exploder extends GLSurfaceView implements GLSurfaceView.Renderer, GestureDetector.OnDoubleTapListener, GestureDetector.OnGestureListener {
    ...
    public x7_Exploder(Context context) {
        super(context);
    ...
    private void initialize(GL10 gl) {
        ...
        final String vertexShaderSourceCode =
                "#version 320 es" +
                        ...
                        "void main(void)" +
                        "{" +
                        "gNormal = normalize(mat3(modelViewMatrix) * aNormal);" +
        ...
        final String geometryShaderSourceCode =
                "#version 320 es" +
                        ...
                        "{" +
                        "gl_Position = gl_in[i].gl_Position + vec4(face_normal * explodeFactor, 0.0);" +
                        "oNormal = gNormal[i];" +
                        "oColor = gColor[i];" +
                        "EmitVertex();" +
                        "}" +
    ...
    private void display() {
        ...
        GLES32.glUniform1f(uniforms.explodeFactor, (float) (Math.sin((float) currentTime * 8.0f) * Math.cos((float) currentTime * 6.0f) * 0.7f + 0.1f));
