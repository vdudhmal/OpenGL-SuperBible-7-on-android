...
public class x11_SampleRateShading extends GLSurfaceView implements GLSurfaceView.Renderer, GestureDetector.OnDoubleTapListener, GestureDetector.OnGestureListener {
    private final float[] perspectiveProjectionMatrix = new float[16];
    private final Timer timer = new Timer();
    boolean sampleShading = false;
    ...
    public x11_SampleRateShading(Context context) {
        super(context);
    ...
    private void initialize(GL10 gl) {
        ...
        final String vertexShaderSourceCode =
                "#version 320 es" +
                        ...
                        "oTextureCoordinates = aVertex.xy;" +
                        "}";
        ...
        final String fragmentShaderSourceCode =
                "#version 320 es" +
                        ...
                        "void main(void)" +
                        "{" +
                        "float val = abs(oTextureCoordinates.x + oTextureCoordinates.y) * 30.0f;" +
                        "fragColor = vec4(fract(val) >= 0.5 ? 1.0 : 0.2);" +
                        "}";
    ...
    private void display() {
        ...
        if (sampleShading) {
            ...
            GLES32.glEnable(GLES32.GL_SAMPLE_SHADING);

            // Set minimum sample shading to 100%
            GLES32.glMinSampleShading(1.0f);
        } else {
            // Disable sample shading
            GLES32.glDisable(GLES32.GL_SAMPLE_SHADING);
        }
