...
public class x1_Perspective extends GLSurfaceView implements GLSurfaceView.Renderer, GestureDetector.OnDoubleTapListener, GestureDetector.OnGestureListener {
    ...
    private boolean use_perspective = false;

    public x1_Perspective(Context context) {
        super(context);
    ...
    private void initialize(GL10 gl) {
        ...
        final String vertexShaderSourceCode =
                "#version 320 es" +
                        ...
                        "noperspective out vec2 oTextureCoordinates_np;" +
                        "void main(void)" +
                        "{" +
                        "const vec4 vertices[] = vec4[](vec4(-0.5, -0.5, 0.0, 1.0), vec4( 0.5, -0.5, 0.0, 1.0), vec4(-0.5,  0.5, 0.0, 1.0), vec4( 0.5,  0.5, 0.0, 1.0));" +
                        "vec2 texture_coordinates = (vertices[gl_VertexID].xy + vec2(0.5));" +
                        "oTextureCoordinates = texture_coordinates;" +
                        "oTextureCoordinates_np = texture_coordinates;" +
                        "gl_Position = modelViewProjectionMatrix * vertices[gl_VertexID];" +
                        "}";
        ...
        final String fragmentShaderSourceCode =
                "#version 320 es" +
                        ...
                        "noperspective in vec2 oTextureCoordinates_np;" +
                        "uniform sampler2D textureSampler;" +
                        "uniform int usePerspective;" +
                        "out vec4 fragColor;" +
                        "void main(void)" +
                        "{" +
                        "vec2 texture_coordinates = mix(oTextureCoordinates_np, oTextureCoordinates, bvec2(usePerspective));" +
                        "fragColor = texture(textureSampler, texture_coordinates).rrrr;" +
                        "}";
        ...
        texture[0] = textureHelper.generateCheckerBoard();
    ...
    private void display() {
        ...
        float t = (float) currentTime * 14.3f;
        ...
        Matrix.rotateM(modelViewMatrix, 0, t, 0.0f, 1.0f, 0.0f);
        ...
        GLES32.glUniform1i(uniforms.usePerspective, use_perspective ? 1 : 0);
    ...
    private static class Uniforms {
        ...
        private void loadUniformLocations(int programId) {
            ...
            usePerspective = GLES32.glGetUniformLocation(programId, "usePerspective");
        }
    }
